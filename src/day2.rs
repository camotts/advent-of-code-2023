...
pub fn input_generator(input: &str) -> Output {
    input
        ...
        .map(|s| {
            ...
            Game {
                id: split[0].split(" ").collect::<Vec<&str>>()[1]
                    .parse::<i32>()
                    .unwrap(),
                pulls: split[1]
                    .split(";")
                    ...
                    .map(|g| {
                        ...
                        let tuples: Vec<(String, i32)> = draws
                            .iter()
                            .map(|d| {
            ...
            g.pulls
                ...
                .fold(default, |acc, p| {
                    let ret: HashMap<String, i32> = HashMap::from([
                        (
                            "red".to_string(),
                            cmp::max(*p.get("red").or(Some(&0)).unwrap(), acc["red"]),
                        ),
                        (
                            "green".to_string(),
                            cmp::max(*p.get("green").or(Some(&0)).unwrap(), acc["green"]),
                        ),
                        (
                            "blue".to_string(),
                            cmp::max(*p.get("blue").or(Some(&0)).unwrap(), acc["blue"]),
                        ),
                    ]);
                    ret
                })
